...
export const withdrawEarn = async ({
  ...
  if (
    ...
    throw new Error(
      `Withdrawal amount is too small. Minimum withdrawal is approximately $${parseFloat(
        ...
        6,
      )}. The contract cannot process such a small amount due to rounding limitations.`,
    );
  }
  ...
  try {
    ...
    console.warn(
      '[withdrawEarn] getWithdrawAmountOut failed with calculated stopLossCost, trying with 0:',
      errorMessage,
    );
    try {
      const result = await earnPoolChecker.simulate.getWithdrawAmountOut([
      ...
      if (
        ...
        fallbackMessage.includes('reverted')
      ) {
        ...
        if (
          ...
          // Calculate minimum withdrawCost needed: ceil(positionSize / percents)
          const minWithdrawCost =
            (positionSize + percents - BigInt(1)) / percents;
          const minWithdrawUsd =
            (minWithdrawCost * price) / BigInt(10 ** cube.stableDecimals);
          const minWithdrawFormatted = formatUnits(minWithdrawUsd, 18);
          throw new Error(
            `Withdrawal amount is too small. Minimum withdrawal is approximately $${parseFloat(
              ...
              6,
            )}. The contract cannot process such a small amount due to rounding limitations (partToWithdraw would be 0).`,
          );
        }
        ...
        throw new Error(
          `Contract simulation failed. This usually means the withdrawal amount is too small or there's an issue with the position. Please try a larger withdrawal amount or contact support if the problem persists.`,
        );
      }

      throw new Error(
        `Failed to estimate withdrawal amount: ${fallbackMessage}. Please try a different withdrawal amount.`,
      );
    }
  }
  ...
  const shouldReservedBeReturned =
    (isFullWithdrawal || isStopLossZero) && reserved > BigInt(0);
  ...
  if (stable !== tokenTo) {
    ...
    try {
      ...
      if (errorMessage.includes('insufficient liquidity')) {
        throw new Error(
          'Insufficient liquidity for this swap. Please try withdrawing to stable token instead.',
        );
      }

      throw new Error(
        '1inch API is unavailable. Please try withdrawing to stable token or try again later.',
      );
    }
  }
